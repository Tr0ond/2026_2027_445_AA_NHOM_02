      ...
      <div class="h-full px-4 sm:px-6 flex items-center justify-between gap-3">
        <div class="min-w-0">
          <span class="font-bold truncate inline-flex items-center gap-2">
            <span class="w-8 h-8 rounded-lg bg-brand-600 flex items-center justify-center text-sm"><i
                class="fa-solid fa-graduation-cap"></i></span>
            {{ phong.ten_lop }}
          </span>
          <span class="text-slate-400 ml-2 text-sm hidden sm:inline">{{ phong.mon_hoc }} · {{
              phong.ngay_hoc
            }} {{ phong.gio_bat_dau }}</span>
        ...
        <div class="flex items-center gap-2">
          <div class="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-800 rounded-lg border border-slate-700">
            <span class="w-2 h-2 bg-red-500 rounded-full animate-pulse"></span><span
              class="text-slate-300 text-xs font-mono">{{ thoiLuongPhongHienThi }}</span></div>
          <div
              class="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-red-500/20 border border-red-500/30 rounded-lg text-red-400 text-xs font-medium">
            <i class="fa-solid fa-circle text-[8px] animate-pulse"></i>REC
          </div>
          ...
          <button v-else
                  class="flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-sm font-semibold bg-slate-700 hover:bg-rose-600 text-slate-200 hover:text-white"
                  @click="roiPhong"><i class="fa-solid fa-phone-slash"></i><span
              class="hidden sm:inline">Rời phòng</span></button>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-1 overflow-hidden min-h-0">
      ...
      <div class="flex-1 min-w-0 flex flex-col p-3 gap-3 overflow-y-auto">
        <div ref="vungVideo" class="relative bg-black rounded-2xl overflow-hidden flex-1 min-h-[360px]">
          ...
          <div class="absolute top-3 right-3 z-[25] flex rounded-xl overflow-hidden border border-white/10">
            ...
            <button class="px-3 py-1.5 text-xs bg-slate-800/90 text-white hover:bg-slate-700" @click="toanManHinh"
                    title="Toàn màn hình">
          ...
          <div v-if="!thongTinAgora?.app_id"
            ...
            <div class="text-sm mt-2 text-slate-500">Lấy App ID miễn phí tại console.agora.io → dán vào BE/.env → khởi
              động lại backend
            </div>
          </div>
          ...
          <div v-else class="khung-o h-full" :class="cheDoXem === 'loi' ? 'che-do-loi' : 'che-do-luoi'">
            ...
            >
              ...
              <div v-if="tv.dangChiaSe" class="nhan bg-amber-400 text-slate-900 !absolute top-1.5 left-1.5"
                   style="z-index: 5">
              ...
              <div class="o-nhan">
                ...
                <i v-else class="fa-solid fa-microphone mr-1"
                   :class="tv.uid === dangNoiUid ? 'text-emerald-400' : ''"></i>
          ...
          <div v-if="hienQr" class="qr-overlay flex-col text-center p-6">
            ...
            <img v-if="qrDataUrl" :src="qrDataUrl" alt="Mã QR điểm danh" width="220" height="220"
                 class="bg-white rounded-2xl p-2"/>
            <div class="text-4xl font-extrabold mt-4 font-mono"
                 :class="conLaiGiay <= 30 ? 'text-rose-500' : 'text-white'">
              {{ demNguoc }}
            </div>
          ...
          <div v-if="!laGiangVien && daXinPhepVang"
               class="absolute inset-0 z-20 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm p-6">
            <div class="max-w-sm text-center"><span
                class="w-20 h-20 mx-auto rounded-3xl bg-sky-500/20 border border-sky-500/30 text-sky-400 flex items-center justify-center text-3xl"><i
                class="fa-solid fa-file-circle-check"></i></span>
              <h3 class="text-xl font-bold text-white mt-4">Đã xin phép vắng</h3>
              <p class="text-sm text-slate-400 mt-2">Đơn của bạn đã được giảng viên duyệt. Bạn không cần quét mã QR điểm
                danh cho buổi học này.</p></div>
          </div>
        </div>

        <!-- Thanh điều khiển media -->
        <div class="flex flex-wrap justify-center gap-2 sm:gap-3 shrink-0">
          <button class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs"
                  :class="micBat ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-rose-600 text-white hover:bg-rose-700'"
          ...
          <button class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs"
                  :class="camBat ? 'bg-slate-700 text-white hover:bg-slate-600' : 'bg-rose-600 text-white hover:bg-rose-700'"
          ...
          <button class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs"
                  :class="dangChiaSe ? 'bg-brand-600 text-white hover:bg-brand-700' : 'bg-slate-700 text-slate-200 hover:bg-slate-600'"
            ...
            <i class="fa-solid fa-display"></i><span class="hidden sm:block">{{
              dangChiaSe ? 'Dừng' : 'Chia sẻ'
            }}</span>
            <i v-if="!quyenToi.chia_se && !laGiangVien" class="fa-solid fa-lock text-[10px] opacity-70"></i>
          </button>

          <button v-if="!laGiangVien"
                  class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs"
          ...
          <button v-if="laGiangVien"
                  class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs bg-teal-600 hover:bg-teal-700 text-white"
                  @click="tabPhong = 'diem_danh'"><i class="fa-solid fa-qrcode text-lg"></i><span
              class="hidden sm:block">QR</span></button>
          <span class="hidden sm:block w-px h-8 bg-slate-700 self-center"></span>
          <button
              class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs bg-slate-700 hover:bg-slate-600 text-white"
              @click="tabPhong = 'chat'"><i class="fa-solid fa-comment text-lg"></i><span
              class="hidden sm:block">Chat</span></button>
          <button
              class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs bg-slate-700 hover:bg-slate-600 text-white"
              @click="tabPhong = 'thanh_vien'"><i class="fa-solid fa-users text-lg"></i><span class="hidden sm:block">Thành viên</span>
          </button>
          <button
              class="w-14 sm:w-16 h-14 rounded-2xl flex flex-col items-center justify-center gap-1 text-xs bg-slate-700 hover:bg-slate-600 text-white"
              @click="toanManHinh"><i class="fa-solid fa-expand text-lg"></i><span
              class="hidden sm:block">Phóng to</span></button>
        </div>
        ...
        <div v-if="false && laGiangVien" class="rounded-2xl bg-slate-900 border border-slate-800 p-4">
          <div class="flex flex-wrap items-center gap-2 mb-4">
            <button class="nut bg-amber-400 text-slate-900 hover:bg-amber-300 text-sm" @click="moPhienDiemDanh"
                    :disabled="phienHienTai !== null">
            ...
            <button v-if="phienHienTai" class="nut bg-rose-600/90 text-white hover:bg-rose-600 text-sm"
                    @click="dongPhien">
          ...
          <div class="overflow-auto max-h-64 rounded-xl border border-slate-800">
            <table class="w-full text-sm">
              ...
              <tr class="text-left text-xs uppercase text-slate-400">
                <th class="px-3 py-2.5">Mã SV</th>
                <th class="px-3 py-2.5">Họ tên</th>
                <th class="px-3 py-2.5">Trạng thái</th>
                <th class="px-3 py-2.5">Giờ quét</th>
              ...
              <tr v-for="sv in danhSachDiemDanh" :key="sv.ma_sinh_vien" class="border-t border-slate-800">
                ...
                <td class="px-3 py-2"><span class="nhan" :class="mauBadge(sv.trang_thai_diem_danh)">{{
                    tenTrangThai(sv.trang_thai_diem_danh)
                  }}</span></td>
                ...
                <td class="px-3 py-2">
                  <div v-if="phienHienTai" class="flex justify-end gap-1.5">
                    <button v-if="sv.trang_thai_diem_danh === 'chua_diem_danh'"
                            ...
                            @click="diemDanhThuCong(sv)">Thủ công
                    </button>
      ...
      <div class="hidden lg:flex w-72 xl:w-80 shrink-0 flex-col bg-slate-800 border-l border-slate-700/50">
        <div class="flex border-b border-slate-700/50 shrink-0">
          <button v-for="tab in cacTabPhong" :key="tab.k"
                  class="flex-1 flex flex-col items-center gap-0.5 py-3 text-xs font-medium transition-colors"
                  :class="tabPhong === tab.k ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-slate-400 hover:text-slate-200'"
                  @click="tabPhong = tab.k"><i :class="tab.icon"></i>{{ tab.ten }}
          </button>
        </div>
        <div v-if="tabPhong === 'thanh_vien'" class="overflow-hidden flex-1">
          ...
          <div class="divide-y divide-slate-800 max-h-72 overflow-y-auto">
            <div v-for="tv in thanhVien" :key="tv.ma_tai_khoan"
                 class="px-4 py-2.5 flex items-center justify-between gap-2 text-sm">
          ...
          <div class="p-3">
            <div ref="khungChat" class="khung-chat !bg-slate-800/60 p-3 mb-3 text-sm space-y-2">
              ...
              <div v-for="(tn, i) in tinNhans" :key="i">
                <div class="text-xs">
                  <span
                      :class="tn.vai_tro === 'giang_vien' ? 'text-amber-400 font-semibold' : 'text-sky-400 font-semibold'">{{
                      tn.ho_ten
                    }}</span>
                  <span class="text-slate-500 ml-1.5">{{ tn.thoi_gian_gui }}</span>
                </div>
                <div
                    class="inline-block bg-slate-700/70 text-slate-100 rounded-xl rounded-tl-sm px-3 py-1.5 mt-0.5 max-w-[85%]">
                  {{ tn.noi_dung }}
                </div>
              </div>
            </div>
        ...
        <div v-if="tabPhong === 'diem_danh'" class="flex-1 overflow-y-auto p-3">
          <template v-if="laGiangVien">
            <button
                class="w-full flex items-center justify-center gap-2 py-3 mb-4 bg-teal-600 text-white rounded-xl hover:bg-teal-700 text-sm font-medium"
                :disabled="phienHienTai !== null" @click="moPhienDiemDanh"><i class="fa-solid fa-qrcode"></i>Tạo mã QR
              điểm danh
            </button>
            <div class="flex items-center justify-between gap-2 mb-3"><p class="text-xs font-semibold text-slate-300">
              {{ soCoMat }}/{{ danhSachDiemDanh.length }} đã điểm danh</p>
              <div class="flex gap-1"><span v-if="soVangCoPhep" class="nhan bg-sky-500/15 text-sky-400">{{
                  soVangCoPhep
                }} có phép</span><span class="nhan bg-emerald-500/15 text-emerald-400">{{
                  danhSachDiemDanh.length ? Math.round(soCoMat / danhSachDiemDanh.length * 100) : 0
                }}%</span></div>
            </div>
            <div class="space-y-1.5">
              <div v-for="sv in danhSachDiemDanh" :key="sv.ma_sinh_vien"
                   class="flex items-center gap-2.5 px-2.5 py-2 rounded-xl bg-slate-700/40">
                <div class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
                     :class="sv.trang_thai_diem_danh === 'vang_co_phep' ? 'bg-sky-600 text-white' : ['co_mat','di_muon'].includes(sv.trang_thai_diem_danh) ? 'bg-emerald-600 text-white' : 'bg-slate-600 text-slate-300'">
                  <i v-if="sv.trang_thai_diem_danh === 'vang_co_phep'" class="fa-solid fa-file-circle-check"></i><i
                    v-else-if="['co_mat','di_muon'].includes(sv.trang_thai_diem_danh)"
                    class="fa-solid fa-check"></i><span v-else>{{ chuVietHoa(sv.ho_ten) }}</span></div>
                <span class="flex-1 text-xs text-slate-300 truncate">{{ sv.ho_ten }}</span>
                <button v-if="sv.trang_thai_diem_danh === 'chua_diem_danh'"
                        class="text-xs px-2 py-1 bg-teal-600/30 text-teal-400 rounded-lg" @click="diemDanhThuCong(sv)">
                  Điểm danh
                </button>
                <span v-else class="nhan !text-[10px]"
                      :class="mauBadge(sv.trang_thai_diem_danh)">{{ tenTrangThai(sv.trang_thai_diem_danh) }}</span>
              </div>
            </div>
          </template>
          <div v-else class="text-center py-8">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-3"
                 :class="daXinPhepVang ? 'bg-sky-500/20' : daDiemDanhToi ? 'bg-emerald-500/20' : 'bg-amber-500/20'"><i
                :class="daXinPhepVang ? 'fa-solid fa-file-circle-check text-sky-400' : daDiemDanhToi ? 'fa-solid fa-circle-check text-emerald-400' : 'fa-solid fa-qrcode text-amber-400'"
                class="text-3xl"></i></div>
            <p class="font-semibold"
               :class="daXinPhepVang ? 'text-sky-400' : daDiemDanhToi ? 'text-emerald-400' : 'text-slate-300'">{{
                daXinPhepVang ? 'Đã xin phép vắng' : daDiemDanhToi ? 'Đã điểm danh thành công!' : 'Chưa điểm danh'
              }}</p>
            <p class="text-slate-500 text-xs mt-1">{{
                daXinPhepVang ? 'Bạn không cần quét QR cho buổi học này' : 'Quét mã QR do giảng viên tạo để điểm danh'
              }}</p></div>
        </div>
      </div>
    </div>
      ...
      try {
        if (this.dangChiaSe) {
          ...
          if (this.manHinhTrack) {
            try {
              await this.agoraClient.unpublish(this.manHinhTrack)
            } catch {
            }
          ...
          if (this.camTrack) {
            try {
              await this.agoraClient.unpublish(this.camTrack)
            } catch {
            }
          }
    ...
    async giaNhapAgora(t) {
      try {
        ...
        try {
          ...
          this.camBat = false
          this.micBat = false
          this.$toast?.show?.('Không bật được camera/mic của bạn: ' + (loi?.msg || loi?.message || ''), {
            type: 'warning',
            duration: 6000
          })
        }
